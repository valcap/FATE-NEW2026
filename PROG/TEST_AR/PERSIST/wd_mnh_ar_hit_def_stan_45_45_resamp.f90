...
!               WD_MOD_BEF_VY(I,J),WD_MOD_AFT_VX(I,J),WD_MOD_AFT_VY(I,J)
    if((SECONDS(I,J) .ne.INT(NOVAL)).AND.(WD_OBS_VX(I,J).ne.9999.).AND.&
      (WD_OBS_VY(I,J).ne.9999.).AND.(WD_MOD_AFT_VX(I,J).ne.9999.).AND.&
            (WD_MOD_AFT_VY(I,J).ne.9999.))then
...
OPEN(UNIT=90,FILE=CFILE_DATE_WS(I),FORM='FORMATTED',STATUS='OLD',ACCESS='SEQUENTIAL')
  ...
  READ(90,*) PUBEL
    ...
    if((SECONDS(I,J) .ne.INT(NOVAL)).AND.(WS_OBS(I,J).ne.9999.).AND.(WS_MOD_AFT(I,J).ne.9999.))then
    SECONDS_DEF(I,J)=SECONDS(I,J)
...
CALL PGLABEL('OBS: WD (deg)','Persistence: WD (deg)','CLASSIC')
DO I=1,NbNights
 DO J=1,NbLines_TOT
...
CALL PGSLW(3)  ! paper
CALL PGENV(xmin,xmax,xhmin,xhmax,1,0)
CALL PGLABEL('OBS: WD (deg)','Persistence: WD (deg)','WITH AR')
DO I=1,NbNights
 DO J=1,NbLines_TOT
