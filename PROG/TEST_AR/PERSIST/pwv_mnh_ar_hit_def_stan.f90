...
REAL                                 :: MAXPWV,MAXPWV_OBS_1D,xmin,xmax,xhmin,xhmax
...
ALLOCATE(PWV_OBS_DEF(NbNights,NbLines_TOT))
ALLOCATE(PWV_MOD_BEF_DEF(NbNights,NbLines_TOT))
ALLOCATE(PWV_MOD_AFT_DEF(NbNights,NbLines_TOT))
SECONDS(:,:)=int(NOVAL)
...
OPEN(UNIT=90,FILE=CFILE_DATE(I),FORM='FORMATTED',STATUS='OLD',ACCESS='SEQUENTIAL')
  ...
  READ(90,*) PUBEL
    ...
    if((SECONDS(I,J) .ne.INT(NOVAL)).AND.(PWV_OBS(I,J).ne.9999.).AND.(PWV_MOD_AFT(I,J).ne.9999.))then
    SECONDS_DEF(I,J)=SECONDS(I,J)
    PWV_OBS_DEF(I,J)=PWV_OBS(I,J)
    PWV_MOD_BEF_DEF(I,J)=PWV_MOD_BEF(I,J)
    PWV_MOD_AFT_DEF(I,J)=PWV_MOD_AFT(I,J)
    end if
    if((SECONDS(I,J) .lt. STARTMINUTE) .or. (SECONDS(I,J) .gt. ENDMINUTE))  then
    PWV_OBS_DEF(I,J)=NOVAL
    PWV_MOD_BEF_DEF(I,J)=NOVAL
    PWV_MOD_AFT_DEF(I,J)=NOVAL
    endif
    ENDDO
CLOSE(90)
ENDDO
...
ALLOCATE(PWV_OBS_1D(NbLines_TOT_1D))
ALLOCATE(PWV_MOD_BEF_1D(NbLines_TOT_1D))
ALLOCATE(PWV_MOD_AFT_1D(NbLines_TOT_1D))
PWV_OBS_1D(:)=NOVAL
...
DO J=1,NbLines_TOT
K=(I-1)*NbLines_TOT+J
PWV_OBS_1D(K)=PWV_OBS_DEF(I,J)
PWV_MOD_BEF_1D(K)=PWV_MOD_BEF_DEF(I,J)
PWV_MOD_AFT_1D(K)=PWV_MOD_AFT_DEF(I,J)
ENDDO
ENDDO
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
if (MAXPWV .eq. 999.) then
  MAXPWV_OBS_1D=MAXVAL(PWV_OBS_1D)
else
  MAXPWV_OBS_1D=MAXPWV
end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!

PRINT*,'MAXPWV_OBS_1D=',MAXPWV_OBS_1D

ALLOCATE(PWV_OBS_1D_LIM(NbLines_TOT_1D))
ALLOCATE(PWV_MOD_BEF_1D_LIM(NbLines_TOT_1D))
ALLOCATE(PWV_MOD_AFT_1D_LIM(NbLines_TOT_1D))
...
IF(PWV_OBS_1D(I).LE.MAXPWV_OBS_1D)THEN
...
!
CALL BIAS_RMSE_SIGMA(PWV_OBS_1D_LIM,PWV_MOD_BEF_1D_LIM,NbLines_TOT_1D,BIAS_PWV_MOD_BEF,RMSE_PWV_MOD_BEF, &
         SIGMA_PWV_MOD_BEF,INC_PWV_MOD_BEF,NOVAL)
write (*,1002) 'LOGINFO BIAS_PWV_MOD_BEF=',BIAS_PWV_MOD_BEF
write (*,1002) 'LOGINFO RMSE_PWV_MOD_BEF=',RMSE_PWV_MOD_BEF
write (*,1002) 'LOGINFO SIGMA_PWV_MOD_BEF=',SIGMA_PWV_MOD_BEF
...
!
CALL BIAS_RMSE_SIGMA(PWV_OBS_1D_LIM,PWV_MOD_AFT_1D_LIM,NbLines_TOT_1D,BIAS_PWV_MOD_AFT,RMSE_PWV_MOD_AFT, &
         SIGMA_PWV_MOD_AFT,INC_PWV_MOD_AFT,NOVAL)
write (*,1002) 'LOGINFO BIAS_PWV_MOD_AFT=',BIAS_PWV_MOD_AFT
write (*,1002) 'LOGINFO RMSE_PWV_MOD_AFT=',RMSE_PWV_MOD_AFT
write (*,1002) 'LOGINFO SIGMA_PWV_MOD_AFT=',SIGMA_PWV_MOD_AFT
...
POD=NOVAL
TAB_HR=NOVAL
! START OF EDITS BY VALCAP74 @ 9-Feb-2026
! PWV \'e sempre calcolato con le soglie fisse
! questo IF ELSE ENDIF pu\'o essere rimosso
IF ( FIX_THRES ) THEN
  X33_PWV=1.90
  X66_PWV=3.46
ELSE
...
!CALL HIT_RATE(PWV_OBS_1D_FIT,PWV_MOD_BEF_1D_FIT,NbFit,1.90,3.46,NOVAL,TAB_HR,POD,PC,EBD,.false.)
...
!CALL HIT_RATE(PWV_OBS_1D_FIT,PWV_MOD_AFT_1D_FIT,NbFit,1.90,3.46,NOVAL,TAB_HR,POD,PC,EBD,.false.)
! END OF EDITS BY VALCAP74 @ 27-March-2023
...
CALL PGLABEL('OBS: PWV (mm)','Persistence: PWV (mm)','STANDARD CONFIGURATION')
DO I=1,NbNights
 DO J=1,NbLines_TOT
   IF( (PWV_OBS_DEF(I,J).NE.0).AND.(PWV_MOD_BEF_DEF(I,J).NE.0)&
       .AND.(PWV_OBS_DEF(I,J).LE.MAXPWV_OBS_1D))THEN
   CALL PGSLW(4)
   CALL PGPT(1,PWV_OBS_DEF(I,J),PWV_MOD_BEF_DEF(I,J),-1)
   ENDIF
 ENDDO
ENDDO
   CALL PGSCH(1.5)
...
CALL PGSLW(3)  ! paper
CALL PGENV(xmin,xmax,xhmin,xhmax,1,0)
CALL PGLABEL('OBS: PWV (mm)','Persistence: PWV (mm)','WITH AR')
DO I=1,NbNights
 DO J=1,NbLines_TOT
   IF( (PWV_OBS_DEF(I,J).NE.0).AND.(PWV_MOD_AFT_DEF(I,J).NE.0)&
       .AND.(PWV_OBS_DEF(I,J).LE.MAXPWV_OBS_1D))THEN
   CALL PGSLW(4)
   CALL PGPT(1,PWV_OBS_DEF(I,J),PWV_MOD_AFT_DEF(I,J),-1)
   ENDIF
 ENDDO
ENDDO
   CALL PGSCH(1.5)
